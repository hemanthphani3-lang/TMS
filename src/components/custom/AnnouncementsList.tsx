...
export function AnnouncementsList({ announcements, viewerRole }: AnnouncementsListProps) {
  ...
  const getTargetBadge = (target: string) => {
    switch (target) {
      ...
      case 'DEPARTMENT_EMPLOYEES': return <span className="bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400 px-2.5 py-1 rounded-full text-xs font-bold">To: Dept Employees</span>
      default: return null
    }
  }
  ...
  const getSenderBadge = (role: string) => {
    ...
    return <span className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 px-2.5 py-1 rounded-full text-xs font-bold">From: Department</span>
  }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  {getSenderBadge(announcement.sender_role)}
                  {(viewerRole === 'ADMIN' || viewerRole === 'DEPARTMENT') && getTargetBadge(announcement.target_audience)}
